package karyawan

import (
	"api-presensi/internal/dto"
	"bytes"
	"strconv"
	"time"

	"github.com/go-pdf/fpdf"
)

func GenerateReportKaryawan(data []dto.KaryawanResponse) ([]byte, error) {
	pdf := fpdf.New("P", "mm", "A4", "")
	pdf.SetTitle("Laporan Karyawan", false)

	// Footer halaman otomatis
	pdf.SetFooterFunc(func() {
		pdf.SetY(-15)
		pdf.SetFont("Arial", "I", 10)
		pdf.CellFormat(0, 10, "Halaman "+strconv.Itoa(pdf.PageNo()), "", 0, "C", false, 0, "")
	})

	pdf.AddPage()

	// add logo
	pdf.Image("internal/utils/report/pf.jpg", 10, 10, 30, 0, false, "", 0, "")

	// Judul
	pdf.SetFont("Arial", "B", 20)

	// beri spasi agar judul bisa sejajar logo
	pdf.Ln(4)
	pdf.CellFormat(0, 10, "Laporan Data Karyawan", "", 1, "C", false, 0, "") // judul rata tengah
	pdf.Ln(1)                                                                // spasi tambahan jika perlu

	// Tanggal
	pdf.SetFont("Arial", "", 14)
	tanggal := time.Now().Format("02 January 2006")

	// rata tengah
	pdf.CellFormat(0, 10, "Per Tanggal: "+tanggal, "", 1, "C", false, 0, "")
	pdf.Ln(2)

	// Geser posisi Y ke bawah agar logo tidak tertimpa judul / tabel
	pdf.SetY(42)

	// Setbackground color dan font color header
	pdf.SetFillColor(255, 140, 60)  // Latar belakang orange (misalnya)
	pdf.SetTextColor(255, 255, 255) // Teks putih
	pdf.SetDrawColor(0, 0, 0)       // Border hitam

	// Header Tabel
	pdf.SetFont("Arial", "B", 12)
	headers := []string{"No", "ID", "Nama", "Jenjang", "Aktif", "Akun Dibuat"} // judul header
	widths := []float64{11, 32, 73, 22, 22, 30}                                // width masing-masing kolom
	aligns := []string{"C", "C", "L", "C", "C", "C"}                           // text-alignment masing-masing kolom

	for i, str := range headers {
		pdf.CellFormat(widths[i], 10, str, "1", 0, "C", true, 0, "")
	}
	pdf.Ln(-1)

	// Isi Tabel
	pdf.SetTextColor(0, 0, 0) // Supaya isi tabel kembali teks hitam
	pdf.SetFont("Arial", "", 12)

	// Cetak data pada tiap row
	var aktif string
	jmlKaryawan := 0

	for i, karyawan := range data {
		if karyawan.Aktif {
			aktif = "Ya"
		} else {
			aktif = "Tidak"
		}

		row := []string{
			// nomor row
			strconv.Itoa(i + 1),
			karyawan.ID,
			karyawan.Nama,
			karyawan.Jenjang,
			aktif,
			karyawan.CreatedAt.Format("2006-01-02"), // format jadi string
		}

		for j, str := range row {
			pdf.CellFormat(widths[j], 10, str, "1", 0, aligns[j], false, 0, "") // C = Center
		}

		// tambah counter jmlKaryawan
		jmlKaryawan += 1

		pdf.Ln(-1)
	}

	pdf.SetFont("Arial", "B", 12)
	pdf.CellFormat(widths[0]+widths[1]+widths[2]+widths[3]+widths[4], 10, "Total Karyawan:", "1", 0, "R", false, 0, "")
	pdf.CellFormat(widths[5], 10, strconv.Itoa(jmlKaryawan)+" orang", "1", 0, "C", false, 0, "")
	pdf.Ln(-1)

	var buf bytes.Buffer
	err := pdf.Output(&buf)
	if err != nil {
		return nil, err
	}
	return buf.Bytes(), nil
}
